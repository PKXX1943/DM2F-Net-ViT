...
def train(net, optimizer):
    ...
    while curr_iter <= cfgs['iter_num']:
        ...
        for data in train_loader:
            ...
            with amp.autocast():
                x_jf, x_j1, x_j2, x_j3, x_j4, x_vit= net(haze)
                ...
                loss = loss_x_jf + loss_x_j1 + loss_x_j2 + loss_x_j3 + loss_x_j4 + loss_x_vit
                
                if isnan(loss):
                    print('( loss: NAN ) iter: ', curr_iter)
            ...
            if curr_iter == 1 or (curr_iter+1)%cfgs['log_freq'] == 0:
                ...
                log = '[iter %d], [train loss %.5f], [loss_x_fusion %.5f], [loss_x_j1 %.5f], ' \
                  '[loss_x_j2 %.5f], [loss_x_j3 %.5f], [loss_x_j4 %.5f], [loss_x_vit %.5f], [lr %.13f]' % \
                  (curr_iter, train_loss_record.avg, loss_x_jf_record.avg,
                   ...
                   loss_x_vit_record.avg, optimizer.param_groups[1]['lr'])
                print(log)
