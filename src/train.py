...
def train(net, optimizer):
    ...
    while curr_iter <= cfgs['iter_num']:
        ...
        loss_x_vit_record = AvgMeter()
        loss_t_vit_record, loss_a_vit_record = AvgMeter(), AvgMeter()
        ...
        for data in train_loader:
            ...
            x_jf, x_j0, x_j1, x_j2, x_j3, x_j4 , x_vit, t, a, t_vit, a_vit= net(haze)
            ...
            loss_x_vit = criterion(x_vit, gt)

            loss_t = criterion(t, gt_trans_map)
            loss_a = criterion(a, gt_ato)
            loss_t_vit = criterion(t_vit, gt_trans_map)
            loss_a_vit = criterion(a_vit, gt_ato)

            loss = loss_x_jf + loss_x_j0 + loss_x_j1 + loss_x_j2 + loss_x_j3 + loss_x_j4 + loss_x_vit + 5 * loss_t + 0.5*loss_a + 5 * loss_t_vit + 0.5*loss_a_vit
            loss.backward()
            ...
            loss_x_j3_record.update(loss_x_j1.item(), batch_size)
            loss_x_j4_record.update(loss_x_j2.item(), batch_size)
            loss_x_vit_record.update(loss_x_vit.item(), batch_size)

            loss_t_record.update(loss_t.item(), batch_size)
            loss_a_record.update(loss_a.item(), batch_size)
            loss_t_vit_record.update(loss_t.item(), batch_size)
            loss_a_vit_record.update(loss_a.item(), batch_size)

            curr_iter += 1

            if curr_iter == 1 or (curr_iter+1)%cfgs['log_freq'] == 0:
                log = '[iter %d], [train loss %.5f], [loss_x_fusion %.5f], [loss_x_phy %.5f], [loss_x_j1 %.5f], ' \
                      ...
                      (curr_iter, train_loss_record.avg, loss_x_jf_record.avg, loss_x_j0_record.avg,
                       ...
                       (loss_t_record.avg+loss_t_vit_record.avg)/2, (loss_a_record.avg+loss_a_vit_record.avg)/2, 
                       optimizer.param_groups[1]['lr'])
                print(log)
                open(log_path, 'a').write(log + '\n')

            if (curr_iter ) % cfgs['val_freq'] == 0:
